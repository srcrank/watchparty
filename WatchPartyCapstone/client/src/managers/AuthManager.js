...
const _apiBaseUrl = "https://localhost:5001";

const _doesUserExist = (firebaseUserId) => {
  return getToken().then((token) =>
    fetch(`${_apiBaseUrl}/api/userprofile/DoesUserExist/${firebaseUserId}`, {
      method: "GET",
      headers: {
...
const _saveUser = (userProfile) => {
  return getToken().then((token) =>
    fetch(`${_apiBaseUrl}/api/userprofile`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  return firebase.auth().signInWithEmailAndPassword(email, pw)
    ...
    .then((doesUserExist) => {
      if (!doesUserExist) {
        ...
        sessionStorage.setItem("userId", doesUserExist.id)
      }
    }).catch(err => {
      console.error(err);
      throw err;
